 ...
 .action
  ...
  function uploadImage(product_id)
  {
	...
	$('#image-upload > [name="file"]').on("change",function()
	{
		...
		$.ajax({
                url : "index.php?route=extension/module/imagechecker/upload&token=<?php print $token; ?>&t="+Date.now(),
                type : 'POST',
                data : formData,
                contentType : false,
                processData : false,
  ...
  <div class="container-fluid">
   <div class="table-responsive">
    <table class="table table-bordered table-hover">
		...
		<tr>
			<form id="filter" method="POST" action="index.php?route=extension/module/imagechecker&token=<?php print $token;?>">
			<td></td>
			<td></td>
			<td class="center">
